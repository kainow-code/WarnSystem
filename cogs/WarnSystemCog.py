...
class WarnSystem(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.db_path = 'database/warnings.db'
    ...
    async def warn(self, ctx, user: discord.Member, *, reason: str):
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                cursor = await db.execute('SELECT punishment_type, duration FROM punishment_settings WHERE warn_count = ?', (warn_count,))
            ...
            if punishment:
                ...
                if punishment_type == "kick":
                    ...
                    embed.add_field(name="Automatische Strafe", value=f"Benutzer wurde für {duration} Minuten stummgeschaltet", inline=False)
    ...
    async def warnings(self, ctx, user: discord.Member):
        try:
            ...
            for warn_id, moderator_id, reason, timestamp in warnings:
                ...
                formatted_time = datetime.datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S.%f").strftime("%d.%m.%Y %H:%M")
                embed.add_field(
    ...
    @commands.slash_command(name="set_punishment", description="Legt eine Strafe für eine bestimmte Anzahl von Warnungen fest")
    @commands.has_permissions(administrator=True)
    async def set_punishment(self, ctx, warn_count: int, punishment_type: Option(str, "kick, ban, mute, none"), duration: int = 0):
        try:
            ...
            if punishment_type == "none":
                ...
                await ctx.respond(f"Strafe für {warn_count} Warnungen festgelegt: {punishment_type}" + (f" für {duration} Minuten" if punishment_type == "mute" else ""))
        except Exception as e:
    ...
    async def show_punishments(self, ctx):
        try:
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute('SELECT warn_count, punishment_type, duration FROM punishment_settings ORDER BY warn_count') as cursor:
